...
async function extractDate() {
    ...
    if (!panoId) {
        showError("Could not cleanly extract a 22-character Pano ID. Try pasting just the 22-character ID itself.");
    ...
    try {
        ...
        if (exactUnix >= ceilingLimit - 10) {
            logInfo(`\n⚠️ Search hit the absolute ceiling limit. Exact time unavailable.`);
            ...
            resultDiv.innerHTML = `
                <div class="warning-box">
                    ...
                    Known Capture Month:<span class="success" style="color: #333; text-align:center;">${fallbackMonth} ${baseDate.year}</span>
                    <div style="font-size: 0.85rem; color: #666; margin-top: 15px;">💡 <b>Tip:</b> Move one click down the street and try the new URL!</div>
                </div>
            `;
        } else {
